...
B<afs/localauth> and B<afs/extauth> are disabled. The default value is
...
If enabled, FABS will skip its own authentication setup and assume that valid
AFS tokens are already present in the execution environment. This is useful
for environments where authentication is managed externally. Compared to the
default keytab-based authentication, this reduces how often we need to contact
the krb5 KDC, avoiding potential throttling for busier installations.

This option takes precedence over both B<afs/localauth> and keytab-based
authentication (using B<afs/keytab>). If B<afs/extauth> is enabled, these other
authentication settings will be ignored.
